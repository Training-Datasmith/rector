 ...
 */
final class Boolean_Type_Mapper implements Type_Mapper_Interface
{
    /**
     * @readonly
     */
    ...
    public function map_to_php_parser_node(Type $type, string $type_kind): ?Node
    {
        ...
        if ($type_kind === Type_Kind::UNION && $type->is_false()->yes()) {
            return new Identifier('false');
        }
        if (!$this->php_version_provider->is_at_least_php_version(Php_Version_Feature::NULL_FALSE_TRUE_STANDALONE_TYPE)) {
            return new Identifier('bool');
        }
        if ($type->is_true()->yes()) {
